...
exec_stats runSemaphore(matrix_2d * matrix2d, int number, int cpu, int user, int execution_number) {
    ...
    printf("Custom Semaphore : \n");
    for (unsigned i = 0; i < nb_iteration; ++i) {
        reset_matrix(matrix2d);
        reset_buffer(matrix2d);
        stats_array[i] = run_one_semaphore(matrix2d, number, cpu, user, execution_number);
        //printf("%d iteration -> %.4fs(cpu), %.4fs (user)\n", (i+1), stats_array[i].execution_time_cpu, stats_array[i].execution_time_user);
    }
    exec_stats stats = compute_average(stats_array, nb_iteration);
    destroy_semaphore(semaphore_impl);
    return stats;
}
...
exec_stats run_one_semaphore(matrix_2d *matrix2d, int thread_number, int measure_cpu, int measure_usr, int execution_number) {
    ...
    // init barriers
    semaphore_impl = init_semaphore(thread_total);
    ...
    for(int i = 0; i < thread_total; i++){
        ...
        if (pthread_create(&array[i], NULL, (void*) main_semaphore_thread, array_chunk[i]) != 0) {
            fprintf(stderr, "error: Cannot create custom thread # %d\n", i);
            break;
        }
    }
